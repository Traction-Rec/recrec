...
public class ComprehensiveValidationTest {
    ...
    public void testComprehensiveValidation() {
        ...
        // Verify auto-fixable issues (only whitespace, NOT scientific notation)
        List<ValidationIssue> autoFixableIssues = result.getAutoFixableIssues();

        // Scientific notation should NOT be auto-fixable (precision risk)
        assertFalse(autoFixableIssues.stream().anyMatch(issue ->
            issue.getType() == IssueType.SCIENTIFIC_NOTATION),
            "Scientific notation should not be auto-fixable due to precision loss risk");

        // Only whitespace issues should be auto-fixable
